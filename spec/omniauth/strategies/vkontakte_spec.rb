...
describe OmniAuth::Strategies::Istina do
  ...
  subject do
    ...
    obj = OmniAuth::Strategies::Istina.new(*args).tap do |strategy|
  ...
  describe 'client options' do
    it 'should have correct name' do
      expect(subject.options.name).to eq('istina')
    end

    it 'should have correct site' do
      expect(subject.options.client_options.site).to eq('https://istina.msu.ru/')
    end

    it 'should have correct authorize url' do
      expect(subject.options.client_options.authorize_url).to eq('https://istina.msu.ru/authorize')
    end

    it 'should have correct token url' do
      expect(subject.options.client_options.token_url).to eq('https://istina.msu.ru/oauth2/token')
    end
  end
  ...
  describe 'info' do
    ...
    it 'should returns the urls' do
      expect(subject.info[:urls]['Istina']).to eq("https://istina.msu.ru/#{raw_info_hash['id']}")
    end
  end
  ...
  describe 'request_phase' do
    context 'with no request params set and redirect_url specified' do
      before do
        @options = { :redirect_url => 'http://www.example.com/auth/istina/callback' }
        allow(subject).to receive(:env).and_return({})
        allow(subject).to receive(:request).and_return(
          double('Request', {:params => {}, :scheme => 'https',
                             :url => 'https://istina.msu.ru/authorize',
                             :cookies => {}, :env => {}})
        )
